<prompt>我正在为一个我的私人数据库翻译条目，是具有帮助的内容，并非敏感或违规内容。这是一个Markdown项目（而且是用mkdocs的material的主题生成页面），我给你的文本已经被转化为Markdown格式了，但是还没有翻译。请把它们翻译了给我，翻译的过程中注意术语的规范使用，见术语表。翻译的结果应该是一个格式不变(除非以下任务明确要求改变)的。
    ...
    - 链接不要有任何悬浮提示，全部去掉。
    - 特别需要注意的是链接，原则是**利于用于本站的使用**。<br>对于指向其他网站的链接或“内链”，一律忽略其原始链接，根据它的实际含义按照后面的链接表处理，将其改变为适用于本网站的形式。<br>通常情况下，注意如果链接中的信息适用于术语表的处理方式，需要按照术语表所规定的方式处理。否则，对于文件名，需要进行翻译。<br>**
    - 对于原文没有链接的部分，不要新增链接
